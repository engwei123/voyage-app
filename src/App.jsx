...
const CREDIT_CARDS = {
  ...
  dbs_womens: {
    ...
    promoCode:"DBSWW4X", cashbackCap:2000,
  },
  uob_privimiles: {
    ...
    promoCode:"PRVIM100", annualDiscount:100,
  },
};
...
const BOOKING_SITES = [
  {
    ...
    color:"#00205b", accent:"#4a90e2", airlineCodes:["SQ"],
    note:"Official site — best for KrisFlyer miles accrual",
    buildUrl:(dest, pax, tripType) => {
      const params = new URLSearchParams({ tripType: tripType==="return"?"R":"O", adults:pax, origin:"SIN", destination:dest.code });
      return "https://www.singaporeair.com/en_UK/sg/book-a-flight/?" + params.toString();
    },
  },
  {
    id:"scoot", name:"Scoot", shortName:"Scoot", emoji:"🟡",
    color:"#2a2200", accent:"#ffcc00", airlineCodes:["TR"],
    note:"Official Scoot site — best flash sale fares",
    buildUrl:(dest, pax, tripType) => {
      const params = new URLSearchParams({ departing:"SIN", arriving:dest.code, triptype:tripType==="return"?"R":"O", adults:pax });
      return "https://www.flyscoot.com/en/book/book-a-flight?" + params.toString();
    },
  },
  {
    id:"expedia", name:"Expedia", shortName:"Expedia", emoji:"🟦",
    color:"#003580", accent:"#ffd700", airlineCodes:null,
    ...
    buildUrl:(dest, pax, tripType) => {
      ...
      if (tripType==="return") params.set("leg2","from:"+dest.code+",to:SIN,departure:"+fmt(ret)+"TANYT");
      return "https://www.expedia.com.sg/Flights-Search?" + params.toString();
    },
  },
  {
    id:"tripcom", name:"Trip.com", shortName:"Trip.com", emoji:"🔵",
    color:"#007aff", accent:"#34aadc", airlineCodes:null,
    note:"Often cheapest for Asia-Pacific routes",
    buildUrl:(dest, pax, tripType) => {
      const params = new URLSearchParams({ adult:pax, type:tripType==="return"?"RT":"OW" });
      return "https://sg.trip.com/flights/sin-to-"+dest.code.toLowerCase()+"/?" + params.toString();
    },
  },
  {
    id:"kayak", name:"Kayak", shortName:"Kayak", emoji:"🛶",
    color:"#ff690f", accent:"#ff8c3a", airlineCodes:null,
    ...
    buildUrl:(dest, pax, tripType) => {
      ...
      return "https://www.kayak.com.sg/flights/"+route+"/"+pax+"adults";
    },
  },
  {
    id:"google", name:"Google Flights", shortName:"Google", emoji:"🔍",
    color:"#1a1a2e", accent:"#4285f4", airlineCodes:null,
    note:"Best for price calendar and fare alerts",
    buildUrl:(dest, pax) => {
      const params = new URLSearchParams({ q:"Flights from Singapore to "+dest.name, curr:"SGD" });
      return "https://www.google.com/travel/flights?" + params.toString();
    },
  },
];

function getBookingSites(alCode, dest, pax, tripType) {
  return BOOKING_SITES
    .slice()
    .sort((a, b) => {
      const aMatch = a.airlineCodes && a.airlineCodes.includes(alCode);
      const bMatch = b.airlineCodes && b.airlineCodes.includes(alCode);
      if (aMatch && !bMatch) return -1;
      if (!aMatch && bMatch) return 1;
      return 0;
    })
    .map(s => ({ ...s, url: s.buildUrl(dest, pax, tripType) }));
}
...
function getBestCard(airlineCode, total) {
  return Object.values(CREDIT_CARDS).map(card => {
    ...
    if (airlineCode==="SQ") {
      discount = total*(card.sqDiscount||0);
      mpdRate = card.mpd.sq||card.mpd.airtickets||card.mpd.online||card.mpd.default;
    } else if (airlineCode==="TR") {
      discount = total*(card.trDiscount||0);
      mpdRate = card.mpd.tr||card.mpd.airtickets||card.mpd.online||card.mpd.default;
    } else {
      mpdRate = card.mpd.airtickets||card.mpd.online||card.mpd.default;
      if (card.annualDiscount) discount = Math.min(card.annualDiscount, total*0.05);
    }
...
function applyAll(basePrice, promo, ccCard, pax) {
  ...
  if (promo) {
    const s = promo.type==="Percentage" ? price*promo.value : Math.min(promo.value*pax, price*0.5);
    price -= s; savings += s;
    applied.push({label:promo.code, saving:s, type:"promo"});
  }
  if (ccCard && ccCard.discount>0) {
    price -= ccCard.discount; savings += ccCard.discount;
    applied.push({label:ccCard.shortName, saving:ccCard.discount, type:"card"});
  }
  return {finalPrice:Math.max(price, basePrice*0.4), savings, applied};
}

function genFlight(dest, pax, src) {
  ...
  return {
    id:al.code+rnd(100,999), al, price, perPax:Math.round(price/pax), pax,
...
function genMonths(dest, pax) {
  ...
  const ms = Array.from({length:12},(_,i)=>{
    ...
    return {month:MON[d.getMonth()],year:d.getFullYear(),idx:i};
  });
  const withP = ms.map(m=>{
    const p=Math.round(base*(0.78+Math.random()*0.62));
    const weeks=Array.from({length:4},(_,w)=>({week:w+1,price:Math.round(p*(0.88+Math.random()*0.26))}));
    return {...m,price:p,weeks};
  });
...
::-webkit-scrollbar{width:3px;height:3px}::-webkit-scrollbar-track{background:transparent}::-webkit-scrollbar-thumb{background:#1c2a42;border-radius:2px}
...
.app{display:flex;flex-direction:column;height:100dvh;height:100vh;background:var(--n0);position:relative;overflow:hidden}
...
.hdr{position:relative;z-index:30;flex-shrink:0;display:flex;align-items:center;padding:0 16px;gap:8px;height:calc(56px + var(--safe-top));padding-top:var(--safe-top);border-bottom:1px solid var(--b1);background:rgba(7,9,15,.95);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px)}
...
.op-pill{display:flex;align-items:center;gap:4px;padding:4px 10px;background:var(--amD);border:1px solid rgba(232,160,0,.2);border-radius:16px;font-family:'IBM Plex Mono',monospace;font-size:9px;color:var(--am);white-space:nowrap}
...
.pb{background:none;border:none;color:var(--am);font-size:16px;width:28px;height:28px;display:flex;align-items:center;justify-content:center;border-radius:6px}
...
.hbtn{width:40px;height:40px;border:1px solid var(--b1);border-radius:8px;background:var(--n2);display:flex;align-items:center;justify-content:center;font-size:16px;position:relative;flex-shrink:0;-webkit-tap-highlight-color:transparent;border:none}
.hbtn:active{background:var(--amD)}
...
.cc-selector{display:flex;gap:5px;overflow-x:auto;-webkit-overflow-scrolling:touch;scrollbar-width:none;padding:8px 14px;border-bottom:1px solid var(--b1);background:rgba(7,9,15,.9);flex-shrink:0}
.cc-selector::-webkit-scrollbar{display:none}
.ccs-btn{display:flex;align-items:center;gap:5px;padding:6px 12px;border-radius:20px;border:1px solid;font-family:'IBM Plex Mono',monospace;font-size:10px;white-space:nowrap;flex-shrink:0;min-height:36px;-webkit-tap-highlight-color:transparent}
.ccs-btn:active{opacity:.8}
.body{display:flex;flex:1;overflow:hidden;position:relative;z-index:10}
.sidebar{width:300px;flex-shrink:0;border-right:1px solid var(--b1);overflow-y:auto;display:flex;flex-direction:column;background:rgba(10,14,23,.7)}
...
.tab{padding:10px 14px;background:none;border:none;font-family:'IBM Plex Sans',sans-serif;font-size:12px;border-bottom:2px solid transparent;margin-bottom:-1px;color:var(--t3);display:flex;align-items:center;gap:4px;white-space:nowrap;flex-shrink:0;min-height:44px;-webkit-tap-highlight-color:transparent}
...
.bottom-nav{display:none;flex-shrink:0;background:rgba(7,9,15,.97);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border-top:1px solid var(--b1);padding-bottom:var(--safe-bot);z-index:30}
.bn-items{display:flex;align-items:stretch}
.bn-item{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;padding:8px 4px;min-height:60px;background:none;border:none;color:var(--t3);font-family:'IBM Plex Sans',sans-serif;font-size:9px;-webkit-tap-highlight-color:transparent;position:relative}
...
.drawer{position:fixed;left:0;top:0;bottom:0;width:min(320px,90vw);z-index:50;background:var(--n1);border-right:1px solid var(--b2);display:flex;flex-direction:column;transform:translateX(-100%);transition:transform .3s cubic-bezier(.4,0,.2,1);overflow-y:auto;-webkit-overflow-scrolling:touch;padding-top:var(--safe-top);padding-left:var(--safe-left);padding-bottom:var(--safe-bot)}
...
.ssa{font-size:9px;color:var(--t3);padding:4px 8px;border-radius:4px;min-height:32px;display:flex;align-items:center;-webkit-tap-highlight-color:transparent;background:none;border:none}
.ssa:active{color:var(--am)}
.addf{display:flex;flex-direction:column;gap:7px}
.ttog{display:flex;gap:4px}
.ttb{flex:1;padding:8px;border-radius:7px;font-size:11px;font-family:'IBM Plex Sans',sans-serif;border:1px solid var(--b1);background:transparent;color:var(--t3);min-height:40px;-webkit-tap-highlight-color:transparent}
.ttb.on{background:var(--amD);border-color:rgba(232,160,0,.32);color:var(--am)}
.ir{display:flex;gap:5px}
.fi{flex:1;background:var(--n1);border:1px solid var(--b1);border-radius:8px;color:var(--t1);font-family:'IBM Plex Sans',sans-serif;font-size:14px;padding:10px 12px;outline:none;transition:border-color .2s;min-height:44px}
...
.addbtn{padding:10px 14px;background:linear-gradient(135deg,var(--am),#b87000);border:none;border-radius:8px;color:#07090f;font-weight:600;font-family:'IBM Plex Sans',sans-serif;font-size:13px;min-height:44px;display:flex;align-items:center;justify-content:center;gap:4px;white-space:nowrap;-webkit-tap-highlight-color:transparent}
.addbtn:active{opacity:.85}
.addbtn:disabled{opacity:.4}
.dlist{display:flex;flex-direction:column;gap:7px;padding:10px 12px}
.dc{background:var(--n1);border:1px solid var(--b1);border-radius:10px;padding:12px 13px;cursor:pointer;position:relative;overflow:hidden;-webkit-tap-highlight-color:transparent}
...
.pbtn{padding:10px 16px;background:linear-gradient(135deg,var(--am),#b87000);border:none;border-radius:8px;color:#07090f;font-weight:600;font-family:'IBM Plex Sans',sans-serif;font-size:12px;min-height:44px;display:flex;align-items:center;gap:6px;-webkit-tap-highlight-color:transparent}
.pbtn:active{opacity:.85}
.pbtn:disabled{opacity:.4}
.gbtn{padding:10px 14px;background:transparent;border:1px solid var(--b1);border-radius:8px;color:var(--t2);font-family:'IBM Plex Sans',sans-serif;font-size:12px;min-height:44px;display:flex;align-items:center;gap:5px;-webkit-tap-highlight-color:transparent}
.gbtn:active{border-color:var(--b2);color:var(--t1)}
.srcbar{display:flex;gap:5px;margin-bottom:12px;overflow-x:auto;-webkit-overflow-scrolling:touch;scrollbar-width:none;padding-bottom:2px}
...
.aa-code{font-family:'IBM Plex Mono',monospace;font-size:11px;padding:4px 9px;background:var(--amD);border:1px solid rgba(232,160,0,.3);border-radius:5px;color:var(--am);min-height:32px;display:flex;align-items:center;gap:3px;-webkit-tap-highlight-color:transparent;border:none}
...
.cc-mini{min-width:100px;padding:8px 10px;border-radius:8px;border:1px solid;flex-shrink:0;-webkit-tap-highlight-color:transparent}
...
.trkbtn{padding:8px 10px;border:1px solid var(--b1);border-radius:7px;background:transparent;color:var(--t3);font-family:'IBM Plex Sans',sans-serif;font-size:14px;min-height:40px;-webkit-tap-highlight-color:transparent}
.trkbtn:active{border-color:var(--am);color:var(--am)}
.trkbtn.on{border-color:rgba(0,196,172,.35);color:var(--te);background:var(--teD)}
.book-btn{padding:9px 16px;background:linear-gradient(135deg,var(--am),#b87000);border:none;border-radius:8px;color:var(--n0);font-weight:700;font-family:'IBM Plex Sans',sans-serif;font-size:12px;min-height:40px;display:flex;align-items:center;gap:5px;-webkit-tap-highlight-color:transparent;white-space:nowrap}
...
.si-chip.cod{border-color:rgba(232,160,0,.28);color:var(--am);background:var(--amB);-webkit-tap-highlight-color:transparent;cursor:pointer}
...
.cc-card{border-radius:13px;padding:15px;position:relative;overflow:hidden;border:1px solid;-webkit-tap-highlight-color:transparent}
...
.bk-sheet{background:var(--n1);border:1px solid var(--b2);width:100%;border-radius:18px 18px 0 0;max-height:92dvh;overflow-y:auto;-webkit-overflow-scrolling:touch;padding-bottom:calc(16px + var(--safe-bot));box-shadow:0 -16px 60px rgba(0,0,0,.7);animation:slideUp .3s cubic-bezier(.4,0,.2,1)}
@keyframes slideUp{from{transform:translateY(40px);opacity:0}to{transform:translateY(0);opacity:1}}
@media(min-width:600px){.bk-sheet{border-radius:16px;width:520px;max-width:95vw;max-height:88vh;padding-bottom:0;animation:fup .25s ease}}
...
.bk-site{display:flex;align-items:center;gap:12px;padding:13px 14px;border-radius:11px;border:1px solid var(--b1);text-decoration:none;-webkit-tap-highlight-color:transparent;position:relative;overflow:hidden}
...
.modal{background:var(--n1);border:1px solid var(--b2);width:100%;border-radius:16px 16px 0 0;padding:20px 18px calc(20px + var(--safe-bot));box-shadow:0 -12px 60px rgba(0,0,0,.7);max-height:90dvh;overflow-y:auto;-webkit-overflow-scrolling:touch;animation:slideUp .3s ease}
@media(min-width:600px){.modal{border-radius:14px;width:440px;max-width:93vw;max-height:90vh;animation:fup .25s ease;padding-bottom:20px}}
...
.akfi{flex:1;background:var(--n2);border:1px solid var(--b1);border-radius:8px;color:var(--t1);font-family:'IBM Plex Mono',monospace;font-size:13px;padding:10px 11px;outline:none;min-height:44px}
...
function useToasts() {
  ...
  const add = useCallback(function(t) {
    const id = Date.now() + Math.random();
    setTs(function(p) { return [...p, Object.assign({}, t, {id:id})]; });
    setTimeout(function() { setTs(function(p) { return p.filter(function(x) { return x.id !== id; }); }); }, 5000);
  }, []);
  return {ts:ts, add:add};
}

function BookingModal({ flight, dest, pax, tripType, onClose }) {
  const [copiedCode, setCopiedCode] = useState(null);
  const sites = getBookingSites(flight.alCode, dest, pax, tripType);
  const allCodes = (flight.applied || [])
    .filter(function(a) { return /^[A-Z0-9]{4,}$/.test(a.label); })
    .map(function(a) { return {code:a.label, desc:a.type==="promo"?"Promo":"CC promo", saving:a.saving}; });
  ...
  return (
    <div className="bk-over" onClick={onClose}>
      <div className="bk-sheet" onClick={function(e){e.stopPropagation();}}>
        ...
        <div className="bk-flight-sum">
          ...
          <div className="bk-route">
            ...
            <div className="bk-route-line">{flight.dep} SIN → {flight.arr} {flight.dest} · {flight.dur}{flight.stops===0?" · Direct":" · "+flight.stops+" stop"+(flight.stops>1?"s":"")}</div>
          </div>
          <div className="bk-price-sum">
            ...
            <div className="bk-pax">{pax} pax · {Math.round(flight.finalPrice/pax).toLocaleString()}/pax</div>
          </div>
        </div>
        {allCodes.length>0 && (
          <div className="bk-codes">
            <div className="bk-sec-lbl">Tap code to copy — paste at checkout</div>
            <div className="bk-code-pills">
              {allCodes.map(function(c,i) {
                return (
                  <div key={i} className="bk-pill" onClick={function(){copy(c.code);}}>
            ...
            <div className="bk-copy-hint"><span>💡</span>Code is auto-copied when you open a booking site below.</div>
          </div>
        )}
        <div className="bk-sites">
          <div className="bk-sec-lbl">Choose booking site</div>
          {sites.map(function(s) {
            const isOfficial = s.airlineCodes && s.airlineCodes.includes(flight.alCode);
            return (
              <a key={s.id} className={"bk-site"+(isOfficial?" primary":"")}
                href={s.url} target="_blank" rel="noopener noreferrer"
                onClick={function(){if(allCodes.length>0) navigator.clipboard && navigator.clipboard.writeText(allCodes[0].code).catch(function(){});}}>
                {isOfficial && <div className="bk-official-badge">★ Official</div>}
                <div className="bk-site-logo" style={{background:s.color+"60",borderColor:s.accent+"40"}}>{s.emoji}</div>
                <div className="bk-site-info">
                  ...
                  {allCodes.length>0 && (
                    <div className="bk-site-code">
                      <span>🏷</span>
                      {allCodes.map(function(c){return <span key={c.code} style={{background:"var(--amD)",padding:"0 4px",borderRadius:3}}>{c.code}</span>;})}
                    </div>
                  )}
                </div>
        ...
        <div className="bk-site-tip">
          ℹ Prices are estimates. Final fare confirmed at checkout. Promo codes are copied to clipboard automatically when you tap a booking site.
        </div>
      </div>
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="ss">
        ...
        <div className="addf">
          <div className="ttog">
            {["one-way","return"].map(function(t) {
              return <button key={t} className={"ttb"+(ntrip===t?" on":"")} onClick={function(){setNtrip(t);}}>{t==="one-way"?"One Way":"↩ Return"}</button>;
            })}
          </div>
          <div className="ir">
            <input className="fi" placeholder="City or airport…" value={ndest} onChange={function(e){setNdest(e.target.value);}} onKeyDown={function(e){if(e.key==="Enter"&&!adding)onAdd();}} />
            <button className="addbtn" onClick={onAdd} disabled={adding||!ndest.trim()}>{adding?<span className="spin dk"/>:"+ Add"}</button>
          </div>
          <div className="tc">
            <div><div className="fl">Budget / pax (SGD)</div><input className="fi sm" type="number" placeholder="e.g. 1200" value={nbudget} onChange={function(e){setNbudget(e.target.value);}}/></div>
      ...
      {scanPct!==null && (
        <div className="scanbar">
          <div className="pw"><div className="pf" style={{width:scanPct+"%"}}/></div>
          <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:8,color:"var(--t3)"}}>Scanning {scanPct}%</div>
        </div>
      )}
      <div className="ss" style={{flex:1,borderBottom:"none"}}>
        <div className="ssl">
          Watching ({dests.length})
          <button className="ssa" onClick={onScanAll}>⟳ Scan all</button>
        </div>
        {dests.length===0 && <div className="empty-dest"><span style={{fontSize:28}}>✈</span>Add a destination above</div>}
        <div className="dlist">
          {dests.map(function(d) {
            ...
            return (
              <div key={d.id} className={"dc"+(d.id===aid?" act":"")} onClick={function(){setAid(d.id);if(onClose)onClose();}}>
                <div className="dct">
                  <div><div className="dcn">{d.name}</div><div className="dcm">{d.code} · {d.country}</div></div>
                  <div className="dcas">
                    <button className="dcb" onClick={function(e){e.stopPropagation();setDests(function(p){return p.filter(function(x){return x.id!==d.id;});});if(aid===d.id){var r=dests.filter(function(x){return x.id!==d.id;});setAid(r[0]?r[0].id:null);}}}>×</button>
                  </div>
                </div>
                <div className="dcpr">
                  <div className="dcpv">{d.price.toLocaleString()}</div>
                  <div className="dcpc">SGD</div>
                  <div className={"pchg"+(chg<0?" pdn":chg>0?" pup":" pfl")}>{chg<0?"▼":chg>0?"▲":"─"}{Math.abs(parseFloat(pct)).toFixed(1)}%</div>
                </div>
                <div className="spark14">
                  {d.priceHistory.map(function(p,i){
                ...
                <div className="dcfoot">
                  ...
                  <div className="trow">
                    <button className={"tog"+(d.alertOn?" on":"")} onClick={function(e){e.stopPropagation();setDests(function(p){return p.map(function(x){return x.id===d.id?Object.assign({},x,{alertOn:!x.alertOn}):x;});});}}>
                      <div className="togk"/>
                    </button>
...
export default function TravelAssistant() {
  ...
  const activeDest = dests.find(function(d){return d.id===aid;});
  const skyLive = apiKeys.sky.length>8;
  const googleLive = apiKeys.serp.length>8;
  const activeSales = [...SALE_DATA.SQ,...SALE_DATA.TR].filter(function(s){return s.status==="active";});
  const upcomingSales = [...SALE_DATA.SQ,...SALE_DATA.TR].filter(function(s){return s.status==="upcoming"&&daysUntil(s.start)<=30;});
  ...
  useEffect(function(){
    scanRef.current=setInterval(function(){runScan();},300000);
    return function(){clearInterval(scanRef.current);};
  },[dests]);

  useEffect(function(){
    activeSales.forEach(function(s){setTimeout(function(){toast({type:"sale",title:"🔥 SALE LIVE — "+s.name,msg:s.discount+(s.code?" · Code: "+s.code:"")});},1500);});
    upcomingSales.slice(0,1).forEach(function(s){setTimeout(function(){toast({type:"al",title:"📅 Upcoming — "+s.name,msg:"Starts "+s.start+" · "+s.discount});},2800);});
  },[]);
  ...
  function loadFlights(dest) {
    setFload(true);setFlights([]);
    setTimeout(function(){
      var srcs=["sim","sim","sim"];
      if(skyLive)srcs.push("skyscanner");
      if(googleLive)srcs.push("google");
      var res=srcs.reduce(function(acc,s){
        for(var i=0;i<rnd(2,3);i++) acc.push(genFlight(dest,pax,s));
        return acc;
      },[]);
      res.push(Object.assign({},genFlight(dest,pax,"sim"),{id:"SQ"+rnd(100,999),al:AIRLINES.find(function(a){return a.code==="SQ";}),alCode:"SQ",al_override:"SQ"}));
      res.push(Object.assign({},genFlight(dest,pax,"sim"),{id:"TR"+rnd(100,999),al:AIRLINES.find(function(a){return a.code==="TR";}),alCode:"TR",al_override:"TR"}));
      setFlights(res.sort(function(a,b){return a.price-b.price;}));
      setFload(false);
    },rnd(700,1400));
  }

  function loadMonths(dest) {
    setMload(true);setMonths([]);
    setTimeout(function(){setMonths(genMonths(dest,pax));setMload(false);},rnd(500,900));
  }

  function loadPromos() {
    setPload(true);setPromos([]);
    setTimeout(function(){
      var all=Object.entries(PROMO_CODES).reduce(function(acc,entry){
        var code=entry[0],list=entry[1];
        var al=AIRLINES.find(function(a){return a.code===code;});
        list.forEach(function(p){acc.push(Object.assign({},p,{airlineCode:code,airlineName:al?al.name:code,airlineEmoji:al?al.emoji:"✈"}));});
        return acc;
      },[]);
  ...
  function refreshPromosAI() {
    ...
    }).catch(function(){loadPromos();}).finally(function(){setPload(false);});
  }
  ...
  function addDest() {
    ...
    fetch("https://api.anthropic.com/v1/messages",{
      method:"POST",headers:{"Content-Type":"application/json"},
      body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:100,messages:[{role:"user",content:"For \""+ndest+"\" return ONLY JSON: {\"name\":\"city\",\"code\":\"IATA\",\"country\":\"country\"}"}]})
    }).then(function(r){return r.json();}).then(function(data){
      var info=JSON.parse(data.content[0].text.replace(/```json|```/g,"").trim());
      createDest(info);
    }).catch(function(){createDest({name:ndest,code:ndest.toUpperCase().slice(0,3),country:""});});
  }

  function createDest(info) {
    var base=rnd(350,1600)*pax;
    var nd={id:Date.now(),name:info.name||ndest,code:info.code||(ndest.toUpperCase().slice(0,3)),country:info.country||"",alertOn:true,budget:nbudget?parseInt(nbudget)*pax:Math.round(base*1.3),tripType:ntrip,price:base,prevPrice:base,priceHistory:gen14Day(base)};
    setDests(function(p){return [...p,nd];});setAid(nd.id);
  ...
  function runScan() {
    ...
    var i=0;
    function next(){
      if(i>=dests.length){setScanPct(null);return;}
      var d=dests[i];
      setScanPct(Math.round(((i+0.5)/dests.length)*100));
      setTimeout(function(){
        var np=Math.round(d.price*(1+(Math.random()-0.46)*0.09));
        setDests(function(p){return p.map(function(x){return x.id===d.id?Object.assign({},x,{prevPrice:x.price,price:np,priceHistory:[...x.priceHistory.slice(-13),np]}):x;});});
        if(np<d.price&&d.alertOn&&np<=d.budget) toast({type:"al",title:"🎯 Price drop — "+d.name,msg:"SGD "+np.toLocaleString()+" · "+pax+" adults · Within budget"});
        i++;next();
      },rnd(250,550));
    }
    next();
  ...
  function copyCode(code){
    navigator.clipboard&&navigator.clipboard.writeText(code).catch(function(){});
    setCopied(code);setTimeout(function(){setCopied(null);},2000);
  ...
  var sortedFlights = flights
    .filter(function(f){return asrc==="all"||f.src===asrc;})
    .map(function(f){
      var code=f.al_override||f.alCode||(f.al?f.al.code:"SQ");
      var promo=getBestPromo(code,f.perPax);
      var ccRankings=getBestCard(code,f.price);
      var myCC=ccRankings.find(function(c){return c.id===activeCCId;});
      var result=applyAll(f.price,promo,myCC,pax);
      return Object.assign({},f,{alCode:code,promo:promo,ccRankings:ccRankings,finalPrice:result.finalPrice,savings:result.savings,applied:result.applied});
    })
    .sort(function(a,b){return a.finalPrice-b.finalPrice;});

  var promoCount = promos.filter(function(p){return !isExp(p.expires);}).length;
  var activeCC = CREDIT_CARDS[activeCCId];
  ...
  return (
    <>
      <style>{CSS}</style>
      <div className="app">
        <div className="grain"/><div className="glow1"/>
        <div className="twrap">
          {ts.map(function(t){return (
        ...
        <div className={"drawer-overlay"+(drawerOpen?" open":"")} onClick={function(){setDrawerOpen(false);}}/>
        <div className={"drawer"+(drawerOpen?" open":"")}>
          <SidebarContent dests={dests} setDests={setDests} aid={aid} setAid={setAid} pax={pax} ndest={ndest} setNdest={setNdest} nbudget={nbudget} setNbudget={setNbudget} ntrip={ntrip} setNtrip={setNtrip} adding={adding} onAdd={addDest} onScanAll={runScan} scanPct={scanPct} onClose={function(){setDrawerOpen(false);}}/>
        </div>
        {selectedFlight&&activeDest&&(
          <BookingModal flight={selectedFlight} dest={activeDest} pax={pax} tripType={activeDest.tripType} onClose={function(){setSelectedFlight(null);}}/>
        )}
        {showSettings&&(
          <div className="mover" onClick={function(){setShowSettings(false);}}>
            <div className="modal" onClick={function(e){e.stopPropagation();}}>
              <div className="modal-handle"/>
              <div className="mtit">⚙ API Connections</div>
              <div className="msub">Connect live flight APIs. Without keys the app uses simulated data. Keys stored in-session only.</div>
              <div className="msec">
                <div className="mst">Skyscanner — RapidAPI</div>
                <div className="akrow"><input className="akfi" placeholder="RapidAPI key…" type="password" value={keyInp.sky} onChange={function(e){setKeyInp(function(p){return Object.assign({},p,{sky:e.target.value});});}}/></div>
                <div className="khint">→ rapidapi.com/skyscanner — requires backend proxy for CORS</div>
                <div className="astr"><div className={"astd"+(skyLive?" ok":" no")}/>{skyLive?"Key saved":"Not connected"}</div>
              </div>
              <div className="msec">
                <div className="mst">Google Flights — SerpAPI</div>
                <div className="akrow"><input className="akfi" placeholder="SerpAPI key…" type="password" value={keyInp.serp} onChange={function(e){setKeyInp(function(p){return Object.assign({},p,{serp:e.target.value});});}}/></div>
              ...
              <div style={{display:"flex",gap:6}}>
                <button className="savebtn" style={{flex:1}} onClick={saveKeys}>Save</button>
                <button className="mcbtn" style={{flex:1}} onClick={function(){setShowSettings(false);}}>Cancel</button>
              </div>
            </div>
          </div>
        )}
        <header className="hdr">
          ...
          <div className="hdr-right">
            ...
            <div className="pctl">
              <button className="pb" onClick={function(){setPax(function(p){return Math.max(1,p-1);});}}>−</button>
              <span className="pn">{pax}</span>
              <button className="pb" onClick={function(){setPax(function(p){return Math.min(9,p+1);});}}>+</button>
              <span style={{fontSize:10,color:"var(--t3)"}}>👤</span>
            </div>
            <button className="hbtn dest-hdr-btn" style={{border:"1px solid var(--b1)"}} onClick={function(){setDrawerOpen(true);}}>🗺</button>
            <button className="hbtn" style={{border:"1px solid var(--b1)"}} onClick={function(){setShowSettings(true);}}>⚙</button>
            <button className="hbtn" style={{border:"1px solid var(--b1)"}} onClick={function(){"Notification" in window&&Notification.requestPermission().then(function(p){if(p==="granted")toast({type:"ok",title:"Alerts enabled",msg:"Price drop alerts active"});});}}>🔔<div className="ndot"/></button>
          </div>
        </header>
        <div className="cc-selector">
          {Object.values(CREDIT_CARDS).map(function(cc){return (
            <button key={cc.id} className="ccs-btn"
              ...
              onClick={function(){setActiveCCId(cc.id);}}>
          ...
          <span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"var(--t3)",padding:"6px 8px",whiteSpace:"nowrap",alignSelf:"center"}}>
            {skyLive?"✓ Sky":"○ Sky"} · {googleLive?"✓ GF":"○ GF"}
          </span>
        </div>
        <div className="body">
          <aside className="sidebar">
            <SidebarContent dests={dests} setDests={setDests} aid={aid} setAid={setAid} pax={pax} ndest={ndest} setNdest={setNdest} nbudget={nbudget} setNbudget={setNbudget} ntrip={ntrip} setNtrip={setNtrip} adding={adding} onAdd={addDest} onScanAll={runScan} scanPct={scanPct}/>
          </aside>
          <main className="panel">
            {!activeDest?(
              <div className="welc">
                ...
                <div className="ws">Tap 🗺 to add a destination and start tracking fares.</div>
              </div>
            ):(
              <>
                <div className="phdr">
                  <div className="pht">
                    ...
                    <div className="pacts">
                      <button className="gbtn" onClick={function(){setShowSettings(true);}}>⚙</button>
                      <button className="pbtn" disabled={fload||mload||pload} onClick={function(){if(tab==="flights")loadFlights(activeDest);else if(tab==="months")loadMonths(activeDest);else if(tab==="promos")refreshPromosAI();}}>
                ...
                <div className="pc">
                  ...
                  {tab==="flights"&&(
                    <>
                      <div className="srcbar">
                        {[{id:"all",l:"All"},{id:"sim",l:"Aggregated"},{id:"skyscanner",l:"✈ Skyscanner"},{id:"google",l:"🔍 Google"}].map(function(s){return (
                          <div key={s.id} className={"schip"+(asrc===s.id?" on":"")} onClick={function(){setAsrc(s.id);}}>{s.l}</div>
                        );}) }
                      </div>
                      {fload?(
                        ...
                        sortedFlights.map(function(f,idx){return (
                          <div key={f.id+idx} className={"fc"+(idx===0?" best":"")} style={{animationDelay:idx*40+"ms"}}>
                            ...
                            <div className="fct">
                              ...
                              <div className="fcr">
                                ...
                                <div className={"fcsrc "+(f.src==="skyscanner"?"sky":f.src==="google"?"google":"sim")}>{f.src==="skyscanner"?"✈ Sky":f.src==="google"?"🔍 GF":"◈ Agg"}</div>
                              </div>
                            </div>
                            {f.applied.length>0&&(
                              <div className="auto-applied">
                                ...
                                <div className="aa-content">
                                  ...
                                  <div className="aa-codes">
                                    {f.applied.map(function(a,i){return (
                                      <span key={i} className="aa-code" onClick={function(){copyCode(a.label);}}>
                            ...
                            <div className="cc-strip">
                              {f.ccRankings.map(function(cc,i){return (
                                <div key={cc.id} className={"cc-mini"+(activeCCId===cc.id?" best-cc":"")} style={{borderColor:activeCCId===cc.id?cc.accent:"var(--b1)",background:activeCCId===cc.id?cc.color+"30":"var(--n2)"}} onClick={function(){setActiveCCId(cc.id);}}>
                            ...
                            <div className="fcbot">
                              ...
                              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                                <button className={"trkbtn"+(tracked[f.id]?" on":"")} onClick={function(){var id=f.id;setTracked(function(p){var n=Object.assign({},p);n[id]=!p[id];return n;});if(!tracked[f.id])toast({type:"ok",title:"Fare tracked",msg:"Alerts if price changes"});}}>
                                  {tracked[f.id]?"✓":"♡"}
                                </button>
                                <button className="book-btn" onClick={function(){setSelectedFlight(f);}}>Book Now ↗</button>
                              </div>
                            </div>
                          </div>
                        );}
                  ...
                  {tab==="sales"&&(
                    <>
                      <div className="sale-hero">
                        {[{code:"SQ",name:"Singapore Airlines",emoji:"🦅",data:SALE_DATA.SQ,cls:"sq"},{code:"TR",name:"Scoot",emoji:"🟡",data:SALE_DATA.TR,cls:"tr"}].map(function(al){
                          var cur=al.data.find(function(s){return s.status==="active";});
                          var next=al.data.find(function(s){return s.status==="upcoming";});
                          return (
                            <div key={al.code} className={"asc "+al.cls}>
                              ...
                              {cur&&<div className="asc-next" style={{color:"var(--te)"}}>{daysUntil(cur.end)} days left</div>}
                              <div style={{display:"flex",alignItems:"center",gap:7,marginTop:10}}>
                                <button className={"tog"+(saleAlerts[al.code]?" on":"")} onClick={function(){setSaleAlerts(function(p){var n=Object.assign({},p);n[al.code]=!p[al.code];return n;});}}>
                                  <div className="togk"/>
                                </button>
                      ...
                      <div className="sale-list">
                        {[
                          ...SALE_DATA.SQ.map(function(s){return Object.assign({},s,{alCode:"SQ",alName:"Singapore Airlines",alEmoji:"🦅"});}),
                          ...SALE_DATA.TR.map(function(s){return Object.assign({},s,{alCode:"TR",alName:"Scoot",alEmoji:"🟡"});})
                        ].sort(function(a,b){var o={active:0,upcoming:1,past:2};return o[a.status]-o[b.status];}).map(function(s,i){
                          var dStart=daysUntil(s.start),dEnd=daysUntil(s.end);
                          return (
                            <div key={i} className={"si"+(s.status==="active"?" active-s":s.status==="upcoming"?" upcoming-s":" past-s")}>
                              ...
                              <div className="si-meta">
                                ...
                                {s.code&&<span className="si-chip cod" onClick={function(){copyCode(s.code);}}>🏷 {s.code} {copied===s.code?"✓":""}</span>}
                              </div>
                  ...
                  {tab==="cards"&&(
                    <>
                      <div className="cc-intro">Select a card to auto-apply its discounts and miles calculation across all flight results.</div>
                      <div className="cc-cards-grid">
                        {Object.values(CREDIT_CARDS).map(function(cc){
                          var calc=getBestCard("SQ",activeDest.price).find(function(c){return c.id===cc.id;});
                          return (
                            <div key={cc.id} className="cc-card"
                              ...
                              {activeCCId===cc.id&&calc&&<div className="cc-disc-badge" style={{background:cc.accent+"20",border:"1px solid "+cc.accent+"40",color:cc.accent}}>✓ Active · {calc.milesEarned.toLocaleString()} mi</div>}
                              <div className="cc-perks">{cc.perks.map(function(p,i){return <div key={i} className="cc-perk" style={{color:cc.accent+"cc"}}>{p}</div>;})}</div>
                            </div>
                          );
                        })}
                      </div>
                      ...
                      <div className="card-comp">
                        {getBestCard("SQ",activeDest.price).map(function(cc,i){return (
                          <div key={cc.id} className={"ccr"+(i===0?" win":"")} onClick={function(){setActiveCCId(cc.id);}}>
                        ...
                        <>
                          <div className="mgrid">
                            {months.map(function(m,i){
                              var maxP=Math.max(...months.map(function(x){return x.price;})),minP=Math.min(...months.map(function(x){return x.price;}));
                              var pct=maxP===minP?50:Math.round(((m.price-minP)/(maxP-minP))*100);
                              return (
                          ...
                          {months.length>0&&(function(){
                            var best=months.find(function(m){return m.isLowest;})||months[0];
                            var minW=Math.min(...best.weeks.map(function(w){return w.price;}));
                            return (
                              <div className="wkgrid">
                                <div className="wgtit">WEEKLY · {best.month.toUpperCase()} {best.year} · {pax} ADULTS</div>
                                <div className="wrow">
                                  {best.weeks.map(function(w,i){
                                    var s=i*7+1;
                                    return (
                  ...
                  {tab==="promos"&&(
                    <>
                      ...
                      {pload?<div className="ldblk"><div className="spin lg"/><div className="ldt">Fetching latest codes…</div></div>:
                        promos.length===0?<div className="eblk">Tap AI Refresh to load codes</div>:(
                        <div className="cgrid">
                          {promos.map(function(p,i){
                            var exp=isExp(p.expires),soon=expSoon(p.expires);
                            return (
                              <div key={i} className="ccard" style={{opacity:exp?0.55:1}}>
                                <div className="cctop">
                                  ...
                                  <div style={{position:"relative"}}>
                                    ...
                                    <div className="codeval" onClick={function(){copyCode(p.code);}}>{p.code}</div>
                                  </div>
                                </div>
                                <div className="cmeta">
                  ...
                  {tab==="history"&&(function(){
                    var h=activeDest.priceHistory;
                    var maxP=Math.max(...h),minP=Math.min(...h);
                    var trend=pctChg(h[h.length-1],h[0]);
                    var days14=Array.from({length:14},function(_,i){var d=new Date();d.setDate(d.getDate()-(13-i));return d.toLocaleDateString("en-SG",{day:"numeric",month:"short"});});
                    var srcs=["Agg","Sky","GF","Sky","Agg","GF","Agg","Sky","GF","Sky","Agg","GF","Sky","Agg"];
                    return (
                      <>
                        <div className="hchart">
                          ...
                          <div className="bchart">
                            {h.map(function(p,i){
                              ...
                              var ht=Math.max(3,Math.round(((p-minP)/range)*44)+3);
                              var isCur=i===h.length-1,isMin=p===minP;
                              return (
                                <div key={i} className="bcc">
                                  ...
                                  <div className="bcd">{i%2===0?days14[i]&&days14[i].split(" ")[0]:""}</div>
                                </div>
                              );
                            })}
                          </div>
                        ...
                        <div className="htable">
                          <div className="htable-inner">
                            ...
                            {h.map(function(p,i){
                              var prev=h[i-1],chg=prev?p-prev:0;
                              var chgPct=prev?pctChg(p,prev):null;
                              var vs=activeDest.budget?p-activeDest.budget:null;
                              return (
            ...
            <nav className="bottom-nav">
              <div className="bn-items">
                {bottomTabs.map(function(t){return (
                  <button key={t.id} className={"bn-item"+(tab===t.id?" on":"")} onClick={function(){setTab(t.id);}}>
                    {t.badge!=null&&<div className={"bn-badge"+(t.badgeClass?" "+t.badgeClass:"")}>{t.badge}</div>}
